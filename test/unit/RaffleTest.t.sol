...
contract RaffleTest is Test, Script {
    ...
    function testDontAllowPlayerToEnterWhileRaffleIsCalculating() public {
        vm.prank(PLAYER);
        raffle.enterRaffle{value: entranceFee}();
        vm.warp(block.timestamp + interval + 1); // ensure that the timeline has passed the deadline
        vm.roll(block.number + 1);
        
        (bool upkeepNeeded, ) = raffle.checkUpkeep("");
        console.log("TIME ELAPSED => 75", upkeepNeeded);
        assertTrue(upkeepNeeded); // Time passed, should now return true
